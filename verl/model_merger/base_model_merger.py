...
class BaseModelMerger(ABC):
    ...
    def get_transformers_auto_model_class(self):
        architectures = getattr(self.model_config, "architectures", [])
        # print(f"architectures: {architectures}")
        if not architectures:
            # 如果没有 architectures 信息，默认使用 AutoModelForCausalLM
            print("Warning: No architectures found in model config, defaulting to AutoModelForCausalLM")
