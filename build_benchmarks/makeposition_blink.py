...
def process_batch(base: datasets.Dataset, start: int, end: int, prefix: str, split: str, processor_id: int, min_size: int = 28):
    results = []
    ...
    save_dir = f'datasets/BLINK_PositionQA{aug_suffix}'
...
def process_split(base: datasets.Dataset, prefix: str, split: str, num_processors: int = None):
    ...
    for i, (base, start, end, prefix, processor_id) in enumerate(jobs):
        ...
        result = process_batch(base, start, end, prefix, split, processor_id)
...
if __name__ == '__main__':
    ...
    directories = [os.path.join(basedir, x) for x in os.listdir(basedir) if os.path.isdir(os.path.join(basedir, x))]
    ...
    for direc in sorted(directories, reverse=True):
        ...
        for split in ['validation', 'test']:
            ...
            logger.info(f"Successfully loaded {split} split with {len(dataset)} items")
            
            try:
                process_split(dataset, prefix, split, num_processors=1)
            except:
                import traceback; traceback.print_exc()
                logger.error(f'Error processing {split} split in {direc}, skipping.')
                continue
