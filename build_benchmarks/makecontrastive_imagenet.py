...
COLUMN_NAMES = ['filename', 'height', 'width', 'label',
    'query', 'answer', 'images']
...
def augment(img) -> Image.Image:
    ...
    cfg, = omegaconf.OmegaConf.load('augcfg_strong.yaml')
...
def process_split(base: datasets.Dataset):
    dlen = len(base)

    # 使用生成器和批处理来减少内存占用
    def generate_pairs():
        # 正样本对
        for dtmid, datum in enumerate(tqdm.tqdm(base, desc='Positive Pairs')):
            img = datum['images'][0]
            try:
                im1 = augment(img)
                im2 = augment(img)
                ...
                pospair = {
                    'filename1': datum['filename'],
                    'height1': im1.height,
                    'width1': im1.width,
                    'label1': datum['label'],
                    'filename2': datum['filename'],
                    'height2': im2.height,
                    'width2': im2.width,
                    'label2': datum['label'],
                    'images': [im1, im2],
        ...
        while neg_count < target_neg_samples:
            ...
            try:
                datum1 = base[i]
                datum2 = base[j]
                img1 = datum1['images'][0]
                img2 = datum2['images'][0]
                ...
                negpair = {
                    'filename1': datum1['filename'],
                    'height1': im1.height,
                    'width1': im1.width,
                    'label1': datum1['label'],
                    'filename2': datum2['filename'],
                    'height2': im2.height,
                    'width2': im2.width,
                    'label2': datum2['label'],
                    'images': [im1, im2],
...
if __name__ == '__main__':
    ...
    source_path = "datasets/imagenet_qa"
    target_path = "datasets/ContrastiveQA_maxcrop03"

    os.makedirs(target_path, exist_ok=True)
    for split in ['train', 'validation']:
        base = datasets.load_dataset(source_path, split=split)
        data_generator = process_split(base)

        # 使用流式保存避免内存溢出，不合并文件
        batch_count = save_dataset_streaming(data_generator, f'{target_path}/{split}', batch_size=10000)
        print(f"Split '{split}' saved as {batch_count} batch files")
