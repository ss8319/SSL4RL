...
class DatasetWrapper:
    def __init__(self, split_path: str, prefix: str):
        ...
        # load image paths
        self.imgpaths = []
        # enumerate all images in the tree with root split_path
        for root, _, files in os.walk(self.split_path):
            for name in files:
                if name.endswith('.json') or name.endswith('.lock') or name.endswith('.metadata') or \
                    name.endswith('.gitattributes') or name.endswith('.jsonl') or name.endswith('.gitignore'):
                    continue
                ...
                try:
                    ...
                    logger(f"Skipping invalid image file: {imgpath}")
                    continue
                except:
                    continue
...
def process_batch(base: DatasetWrapper, start: int, end: int, prefix: str, processor_id: int, seed: int, min_size: int = 28):
    ...
    save_dir = f"/datasets/VStar_JigsawQA_{JIGSAW_SIZE}x{JIGSAW_SIZE}"
...
if __name__ == '__main__':
    ...
    data_base_dir = 'datasets/vstar_bench'
