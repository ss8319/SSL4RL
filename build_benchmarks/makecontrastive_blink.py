...
def process_split(base: datasets.Dataset, split: str):
    dlen = len(base)

    # 使用生成器和批处理来减少内存占用
    def generate_pairs():
        # 正样本对
        progbar = tqdm.tqdm(base, desc='Positive Pairs')
        for dtmid, datum in enumerate(progbar):
        ...
        while neg_count < target_neg_samples:
            ...
            datum1 = base[i]
            datum2 = base[j]
            imglist1 = [im for im in [datum1['image_1'], datum1['image_2'], datum1['image_3'], datum1['image_4']] if im is not None]
            imglist2 = [im for im in [datum2['image_1'], datum2['image_2'], datum2['image_3'], datum2['image_4']] if im is not None]
            subi = random.randint(0, len(imglist1) - 1)
            subj = random.randint(0, len(imglist2) - 1)
            if i >= j:
                continue

            if (i, subi, j, subj) in pool:
                continue
            pool.add((i, subi, j, subj))
            ...
            try:
                ...
                # 强制垃圾回收
                del im1, im2, img1, img2, datum1, datum2
                gc.collect()
...
def save_dataset_streaming(data_generator, output_path, batch_size=10000):
    ...
    for item in data_generator:
        ...
        if len(batch_data) >= batch_size:
            ...
            print(f"Saved batch {batch_count} with {len(batch_data)} samples")
    ...
    if batch_data:
        batch_ds = datasets.Dataset.from_list(batch_data)
        batch_ds.to_parquet(f'{output_path}-{batch_count:05d}.parquet')
        print(f"Saved final batch {batch_count} with {len(batch_data)} samples")
        del batch_data, batch_ds
        gc.collect()
        batch_count += 1
...
if __name__ == '__main__':
    ...
    target_path = "datasets/BLINK_ContrastiveQA"
    os.makedirs(target_path, exist_ok=True)

    for direc in sorted(directories, reverse=True):
        if '___' in direc:
            continue
        dataset_path = direc
        ...
        for split in ['validation', 'test']:
            ...
            try:
                data_generator = process_split(dataset, split)

                # 使用流式保存避免内存溢出，不合并文件
                batch_count = save_dataset_streaming(data_generator, f'{target_path}/{prefix}', batch_size=300)
                print(f"Split {prefix=} {split=} saved as {batch_count} batch files")
